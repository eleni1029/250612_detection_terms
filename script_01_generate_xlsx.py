...
# -*- coding: utf-8 -*-
"""
script_01_generate_xlsx.py (v2.4 - Excel解析修復版本)
...
基於語言獨立橫向分區塊 Excel 生成各語言 tobemodified 檔案
讀取 phrase_comparison.xlsx 中的替換方案，生成針對性修正清單
"""
...
def detect_sensitive_phrases_in_files(config, language: str, sensitive_words: dict):
    """
    在指定語言的翻譯檔案中檢測敏感詞
    ...
    try:
            ...
            if po_path.exists():
                ...
                try:
                    ...
                    for entry in po_data:
                        ...
                        # 檢查翻譯文本中的敏感詞
                        for sensitive_word, word_info in flat_sensitive_words.items():
                            if sensitive_word in entry.msgstr:
        ...
        print(f"   📊 檢測到 {len(detected_items)} 個敏感詞項目")
...
def generate_tobemodified_excel(config, language: str, detected_items: list, output_dir: Path):
    ...
    if not detected_items:
        print(f"   ➡️  語言 {language} 無需修正項目，跳過生成")
        return
    ...
    for bt_code, bt_config in business_types.items():
        ...
        headers.append(f"{bt_config['display_name']}_替換結果")  # 新增替換結果列
    ...
    for row_num, item in enumerate(detected_items, 2):
        ...
        basic_data = [
            ...
            str(item['file_path'].name),  # 只顯示檔案名稱，避免路徑錯誤
            item['entry_id'],
        ...
        for bt_code, bt_config in business_types.items():
            ...
            if replacement and replacement.strip():
                # 只有當有替換方案時才計算替換結果
                # 執行替換
                if sensitive_word in original_text:
                    predicted_result = original_text.replace(sensitive_word, replacement)
                else:
                    # 如果直接替換失敗，嘗試大小寫不敏感替換
                    import re
                    pattern = re.escape(sensitive_word)
                    predicted_result = re.sub(pattern, replacement, original_text, flags=re.IGNORECASE)
    ...
    for col_idx in range(1, len(headers) + 1):
        ...
        for row_idx in range(1, min(ws.max_row + 1, 100)):  # 限制檢查的行數
...
def main():
    ...
    print(f"   檢測到語言：{', '.join(available_languages)}")
    ...
    print(f"✅ 將處理 {len(common_languages)} 個共同語言：{', '.join(sorted(common_languages))}")
    ...
    try:
        ...
        print(f"⚠️  無法獲取輸出目錄配置，使用默認：i18n_output")
        output_dir = Path('i18n_output')
    ...
    for language in sorted(common_languages):
        ...
        # 檢測敏感詞
        detected_items = detect_sensitive_phrases_in_files(config, language, sensitive_words)
        ...
        if detected_items:
            ...
            print(f"   ✅ 語言 {language} 沒有檢測到敏感詞")
    
    # 生成總結報告
    print(f"\n📊 處理完成總結：")
    print(f"   處理語言數：{len(common_languages)}")
    print(f"   檢測項目總數：{total_detected}")
    print(f"   輸出目錄：{output_dir}")
    
    if total_detected > 0:
        print(f"\n✅ 已生成 {len(common_languages)} 個語言的待修正清單")
        print("   下一步：檢查並編輯生成的 tobemodified Excel 檔案")
        print("   最後執行：script_02_apply_fixes.py 套用修正結果")
    else:
        print("✅ 所有語言都沒有檢測到敏感詞，無需修正")
